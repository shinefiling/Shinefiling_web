...
import {
    ...
    Save, Shield, CreditCard, Lock, Info, Award, Smartphone, User, MapPin, Key, X, ArrowRight
} from 'lucide-react';
...
const pricing = {
    standard: { serviceFee: 1999, title: "Standard (2 Years)", subtitle: "Class 3 Signing + Encryption" },
    plus: { serviceFee: 2999, title: "Plus (3 Years)", subtitle: "Best Value + Priority Dispatch" },
    dgft: { serviceFee: 4999, title: "DGFT (2 Years)", subtitle: "For Import/Export (IEC)" }
};

const ApplyDigitalSignature = ({ isLoggedIn, isModal = false, onClose, planProp }) => {
    ...
    // Update validity based on plan selection
    useEffect(() => {
        if (plan === 'plus') {
            setFormData(prev => ({ ...prev, validity: '3 Years' }));
        } else {
    ...
    const validateStep = (currentStep) => {
        ...
        switch (currentStep) {
            ...
            case 2: // Applicant Details
                ...
                if (!formData.aadhaarNumber) return "Aadhaar Number is required.";
                if (!formData.mobile || formData.mobile.length !== 10) return "Valid Mobile Number is required.";
                if (!formData.email) return "Email is required.";
                if (!formData.address) return "Delivery Address is required.";
                return null;
            case 3: // Documents
                if (!documents.panCard) return "PAN Card copy is mandatory.";
                if (!documents.photo) return "Passport Size Photo is mandatory.";
                return null;
            default:
                return null;
        }
    ...
    const handleSubmit = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${billDetails.planName} - Digital Signature`,
            prefill: {
            ...
            onSuccess: async (response) => {
                ...
                try {
                    ...
                    // API Call
                    await submitDigitalSignature(submissionPayload);
    ...
    const steps = ['Plan Selection', 'Applicant Details', 'Documents', 'Review', 'Payment'];
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                                <Key size={20} className="text-sky-600" /> SELECT PLAN
                            </h3>
                            <div className="bg-sky-50/50 p-4 rounded-xl border border-sky-100 flex items-start gap-3 mb-6">
                                <Key className="text-sky-600 mt-0.5 shrink-0" size={18} />
                                <p className="text-sm text-sky-800">
                                    Select validity period. <strong>USB Token</strong> is included with all plans.
                                </p>
                            </div>
                                    ...
                                    {Object.entries(pricing).map(([key, val]) => (
                                        ...
                                        >
                                            ...
                                            <div className="text-right">
                                                <div className="text-sky-600 font-bold">₹{val.serviceFee}</div>
                                                {plan === key && <CheckCircle size={16} className="text-sky-600 ml-auto mt-1" />}
                                            </div>
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Applicant Name</label>
                                    <input type="text" name="applicantName" value={formData.applicantName} onChange={handleChange} placeholder="Same as PAN" className="w-full p-3 rounded-lg border border-gray-200 bg-white" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">User Type</label>
                                    <select name="userType" value={formData.userType} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input type="text" name="panNumber" value={formData.panNumber} onChange={handleChange} maxLength={10} className="w-full p-3 rounded-lg border border-gray-200 bg-white uppercase" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Aadhaar Number</label>
                                    <input type="text" name="aadhaarNumber" value={formData.aadhaarNumber} onChange={handleChange} maxLength={12} className="w-full p-3 rounded-lg border border-gray-200 bg-white" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Linked Mobile</label>
                                ...
                                <div className="space-y-1 md:col-span-2">
                                    <label className="text-xs font-bold text-gray-500">Delivery Address (For Token)</label>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {[
                                    { id: 'panCard', label: 'PAN Card Copy', icon: FileText },
                                    { id: 'photo', label: 'Passport Size Photo', icon: User }
                                ].map((doc) => (
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        ...
                        <div className="max-w-xs mx-auto bg-gray-50 p-6 rounded-2xl mb-8 border border-gray-200 text-left">
                            <div className="space-y-3 text-sm">
                                ...
                                <div className="flex justify-between"><span>Validity</span><span className="font-bold">{formData.validity}</span></div>
                ...
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        ...
                        <div className="max-w-xs mx-auto bg-gray-50 p-6 rounded-2xl mb-8 border border-gray-200">
                            <div className="flex justify-between text-sm mb-2 text-gray-600"><span>DSC Fee ({formData.validity})</span><span>₹{billDetails.base.toLocaleString()}</span></div>
                    ...
                    <div className="flex flex-col justify-center">
                        <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                            <div className="flex items-center gap-2 truncate">
                                <span className="font-bold text-slate-800 text-sm truncate">Digital Signature</span>
                            </div>
