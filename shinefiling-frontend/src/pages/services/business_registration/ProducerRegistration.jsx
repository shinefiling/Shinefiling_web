...
import {
    ...
    User, MapPin, Plus, Trash2, ArrowLeft, ArrowRight, X, Sprout, Tractor, Users, Shield, Receipt, AlertCircle, Lock
} from 'lucide-react';
import { useAuth } from '../../../context/AuthContext';
import { submitProducerRegistration, uploadFile } from '../../../api';
...
const validatePlan = (plan) => {
    ...
    if (['pro', 'standard', 'fpo'].includes(p)) return 'pro';
    if (['elite', 'premium', 'growth'].includes(p)) return 'elite';
    return 'starter';
};

const ProducerRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    const plans = {
        starter: {
            price: 14999,
            title: 'Kisan Basics',
            features: ["5 DSC & 5 DIN", "Name Approval", "MOA & AOA", "Incorporation Cert."],
            color: 'bg-white border-slate-200'
        },
        pro: {
            price: 24999,
            title: 'FPO Standard',
            features: ["Everything in Basics", "PAN & TAN", "GST Registration", "Bank Account Support"],
            color: 'bg-green-50 border-green-200'
        },
        elite: {
            price: 39999,
            title: 'FPO Growth',
            features: ["Everything in Standard", "Nabard Funding Guide", "FPO Compliance (1 Yr)", "Legal Advisory"],
    ...
    const [formData, setFormData] = useState({
        ...
        directors: [
            ...
            { id: 4, name: '', email: '', mobile: '', pan: '', aadhaar: '' },
            { id: 5, name: '', email: '', mobile: '', pan: '', aadhaar: '' }
        ]
    });
    ...
    const addDirector = () => {
        if (formData.directors.length < 10) {
            setFormData({
                ...formData,
    ...
    const removeDirector = (index) => {
        if (formData.directors.length <= 5) {
            alert("Minimum 5 directors required for Producer Company.");
            return;
        }
    ...
    const validateStep = (step) => {
        ...
        if (step === 1) {
            ...
            if (!formData.numberOfProducers) { newErrors.numberOfProducers = "Required"; isValid = false; }
        }
        setErrors(newErrors);
        return isValid;
    };
    ...
    const submitApplication = async () => {
        ...
        const email = userObj?.email || formData.userEmail || formData.directors[0].email;
        const phone = userObj?.phone || formData.userPhone || formData.directors[0].mobile;

        processPayment({
            amount: billDetails.total,
            description: `Payment for ${plans[selectedPlan]?.title} - Producer Registration`,
            prefill: {
            ...
            onSuccess: async (response) => {
                ...
                try {
                    ...
                    const apiResponse = await submitProducerRegistration(finalPayload);
        ...
        switch (currentStep) {
            case 1: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <h3 className="font-bold text-navy mb-3 text-sm flex items-center gap-2"><Tractor size={16} /> FPO DETAILS</h3>
                        <div className="grid md:grid-cols-2 gap-3">
                            ...
                            <select name="activityType" value={formData.activityType} onChange={handleInputChange} className="p-2 text-sm border rounded-lg">
                            ...
                            <input name="registeredAddress" value={formData.registeredAddress} onChange={handleInputChange} placeholder="Farm/Office Address" className={`p-2 text-sm border rounded-lg ${errors.registeredAddress ? 'border-red-500' : ''}`} />
                            <input name="numberOfProducers" type="number" value={formData.numberOfProducers} onChange={handleInputChange} placeholder="Producers Count (Min 10)" className={`col-span-2 p-2 text-sm border rounded-lg ${errors.numberOfProducers ? 'border-red-500' : ''}`} />
                        </div>
                    </div>
                </div>
            );
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    {formData.directors.map((dir, i) => (
                        <div key={dir.id || i} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm relative">
                            <div className="flex justify-between mb-2">
                                ...
                                {formData.directors.length > 5 && <Trash2 size={14} onClick={() => removeDirector(i)} className="cursor-pointer text-red-500" />}
                            </div>
                            <div className="grid md:grid-cols-2 gap-3">
                    ...
                    {formData.directors.length < 10 && <button onClick={addDirector} className="w-full py-3 border-2 border-dashed rounded-xl text-gray-400 font-bold text-xs">+ ADD DIRECTOR</button>}
                </div>
            );
            case 3: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="border border-dashed p-3 rounded-lg flex justify-between items-center bg-gray-50">
                                <span className="text-xs font-bold">Farm/Address Proof</span>
            ...
            case 4: return (
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg space-y-3 text-sm">
                        ...
                        <div className="flex justify-between border-b pb-2"><span>FPO Name</span><span className="font-bold">{formData.companyNameOption1}</span></div>
                        ...
                        <div className="flex justify-between"><span>Activity</span><span className="font-bold">{formData.activityType}</span></div>
                    </div>
                </div>
            );
            case 5: return (
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['FPO Details', 'Directors', 'Documents', 'Review', 'Payment'].map((step, i) => (
                            <div key={i}
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-[#ED6E3F] text-sm truncate">Producer Registration</span>
                                </div>
