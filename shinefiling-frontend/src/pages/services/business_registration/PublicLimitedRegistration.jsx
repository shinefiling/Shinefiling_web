...
import {
    ...
    User, MapPin, Plus, Trash2, ArrowLeft, ArrowRight, X, Building, Users, Landmark, Shield, Receipt, AlertCircle, Lock
} from 'lucide-react';
...
const validatePlan = (plan) => {
    ...
    if (['pro', 'standard'].includes(p)) return 'pro';
    if (['elite', 'premium', 'ultimate'].includes(p)) return 'elite';
    return 'starter';
};
...
const PublicLimitedRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const validateStep = (step) => {
        ...
        if (step === 1) {
            ...
            if (!formData.numberOfShareholders) { newErrors.numberOfShareholders = "Required"; isValid = false; }
        }
        setErrors(newErrors);
        return isValid;
    };
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${plans[selectedPlan]?.title} - Public Limited Registration`,
            prefill: {
            ...
            onSuccess: async (response) => {
                ...
                try {
                    ...
                    const apiResponse = await submitPublicLimitedRegistration(finalPayload);
        ...
        switch (currentStep) {
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="grid md:grid-cols-2 gap-3">
                            ...
                            <select name="businessActivity" value={formData.businessActivity} onChange={handleInputChange} className="p-2 text-sm border rounded-lg">
                            ...
                            <input name="authorizedCapital" value={formData.authorizedCapital} onChange={handleInputChange} placeholder="Auth. Capital (Min 5L)" className="p-2 text-sm border rounded-lg" />
                            <input name="numberOfShareholders" type="number" value={formData.numberOfShareholders} onChange={handleInputChange} placeholder="Shareholders (Min 7)" className={`p-2 text-sm border rounded-lg ${errors.numberOfShareholders ? 'border-red-500' : ''}`} />
                        </div>
                    </div>
                </div>
            );
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    ...
                    {formData.directors.length < 15 && <button onClick={addDirector} className="w-full py-3 border-2 border-dashed rounded-xl text-gray-400 font-bold text-xs">+ ADD DIRECTOR</button>}
                </div>
            );
            case 3: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="border border-dashed p-3 rounded-lg flex justify-between items-center bg-gray-50">
                                <span className="text-xs font-bold">Address/Registered Proof</span>
            ...
            case 4: return (
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg space-y-3 text-sm">
                        ...
                        <div className="flex justify-between border-b pb-2"><span>Entity Name</span><span className="font-bold">{formData.companyNameOption1}</span></div>
                        ...
                        <div className="flex justify-between"><span>Shareholders</span><span className="font-bold">{formData.numberOfShareholders}</span></div>
                    </div>
                </div>
            );
            case 5: return (
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Entity Details', 'Directors', 'Documents', 'Review', 'Payment'].map((step, i) => (
                            <div key={i}
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-[#ED6E3F] text-sm truncate">Public Ltd Registration</span>
                                </div>
                            ...
                            <h2 className="hidden md:block font-bold text-slate-800 text-lg">
                                ...
                                {currentStep === 2 && "Director & Shareholder Info"}
