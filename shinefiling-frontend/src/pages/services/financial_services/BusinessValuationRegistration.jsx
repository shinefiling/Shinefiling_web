...
const BusinessValuationRegistration = ({ isModal, onClose, initialData = {}, planProp }) => {
    ...
    const [formData, setFormData] = useState({
        businessName: '',
        purpose: 'Funding / Equity Dilution',
    ...
    const plans = {
        standard: {
            price: 9999,
            title: 'Standard',
            features: ["Internal Usage Only", "Approx Valuation", "Basic Financial Model"],
            color: 'bg-white border-slate-200'
        },
        certificate: {
            price: 19999,
            title: 'CA Certified',
            features: ["CA Certification (Rule 11UA)", "DCF / NAV Method", "Angel Tax Compliance", "Investor Ready Report"],
            recommended: true,
        ...
        merchant: {
            ...
            features: ["SEBI Regd. Banker Cert.", "FEMA / RBI Guidelines", "For FDI Investment", "Comprehensive Analysis"],
    ...
    const handleFileUpload = async (e, key) => {
        ...
        try {
            const response = await uploadFile(file, 'valuation_docs');
    ...
    const steps = ['Business Details', 'Financial Summary', 'Documents', 'Review & Pay'];
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-slate-800 text-sm truncate">Business Valuation</span>
                                </div>
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2">
                                <Building size={16} className="text-[#015A62]" /> ENTITY BACKGROUND
                            </h3>
                            <div className="grid md:grid-cols-2 gap-3">
                                <div className="md:col-span-2">
                                    <label className="text-xs font-bold text-gray-500 mb-1 block">Company / Brand Name</label>
                                    <input type="text" name="businessName" value={formData.businessName} onChange={handleChange} placeholder="Full Legal Name" className={`w-full p-2 text-sm rounded-lg border bg-white ${errors.businessName ? 'border-red-500' : 'border-gray-200'}`} />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-gray-500 mb-1 block">Valuation Purpose</label>
                                    <select name="purpose" value={formData.purpose} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white">
                                        <option value="Funding / Equity Dilution">Funding / Equity Dilution</option>
                                        <option value="Merger / Acquisition">Merger / Acquisition</option>
                                        <option value="RBI / FEMA Compliance">RBI / FEMA Compliance</option>
                                        <option value="Income Tax Compliance">Income Tax Compliance</option>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="industry" value={formData.industry} onChange={handleChange} placeholder="e.g. EdTech, SaaS" className="w-full p-3 rounded-lg border border-gray-200 bg-white" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                                <DollarSign size={20} className="text-green-600" /> REVENUE & TRACTION
                            </h3>
                            <div className="grid md:grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-bold text-gray-500 mb-1 block">Current Stage</label>
                                    <select name="revenueStage" value={formData.revenueStage} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="lastYearRevenue" value={formData.lastYearRevenue} onChange={handleChange} placeholder="e.g. 15 Lakhs" className="w-full p-3 rounded-lg border border-gray-200 bg-white" />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="text-xs font-bold text-gray-500 mb-1 block">Funding Goal (if applicable)</label>
                                    <input type="text" name="fundingGoal" value={formData.fundingGoal} onChange={handleChange} placeholder="How much are you looking to raise?" className="w-full p-3 rounded-lg border border-gray-200 bg-white" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            case 3: // Documents
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                                <FileText size={20} className="text-bronze" /> FINANCIAL DATA
                            </h3>
                            <p className="text-sm text-gray-500 mb-6">Upload these to help our valuers build the DCF model.</p>

                            <div className="grid md:grid-cols-2 gap-4">
                                {['3 Year Audited Financials', '5 Year Financial Projections', 'Current Cap Table', 'Business Pitch Deck'].map((label, idx) => {
                                    const key = `doc_${idx}`;
                                    return (
            ...
            case 4: // Review
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        ...
                        <p className="text-gray-500 mb-8">Professional valuation by registered experts.</p>
                        ...
                        <div className="text-left bg-gray-50 p-4 rounded-xl mb-6 text-sm space-y-2 border border-gray-200">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-semibold text-xs truncate max-w-[150px]">{formData.purpose}</span>
                            </div>
                        </div>
...
export default BusinessValuationRegistration;
