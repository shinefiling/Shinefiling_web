...
import {
    ...
    ArrowLeft, ArrowRight, IndianRupee, Copyright, BookOpen, AlertTriangle, X
} from 'lucide-react';
...
const ApplyCopyrightRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const [planType, setPlanType] = useState(() => {
        ...
        return ['consultation', 'standard_copyright', 'sound_cinema', 'standard'].includes(target) ? target : 'standard';
    });

    const planFromUrl = searchParams.get('plan');
    useEffect(() => {
        const targetPlan = (planProp || planFromUrl)?.toLowerCase();
        const validPlan = ['consultation', 'standard_copyright', 'sound_cinema', 'standard'].includes(targetPlan) ? targetPlan : 'standard';
    ...
    const plans = {
        ...
        standard_copyright: { title: "Literary / Artistic", serviceFee: 2999 },
        sound_cinema: { title: "Sound / Cinema", serviceFee: 6999 },
        standard: { title: "Literary / Artistic", serviceFee: 2999 },
    };
    ...
    const validateStep = (step) => {
        ...
        let isValid = true;
        if (step === 1) {
            if (!formData.workTitle) { newErrors.workTitle = "Required"; isValid = false; }
            if (!formData.authorName) { newErrors.authorName = "Required"; isValid = false; }
            if (formData.publicationStatus === 'PUBLISHED' && !formData.publicationDate) {
                newErrors.publicationDate = "Required"; isValid = false;
            }
        }
        setErrors(newErrors);
        return isValid;
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${billDetails.planName} - Copyright Registration`,
            prefill: {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Copyright size={16} className="text-orange-500" /> WORK DETAILS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Title of Work</label>
                                <input name="workTitle" value={formData.workTitle} onChange={handleInputChange} placeholder="e.g. My Creative Book" className={`w-full p-2.5 text-sm border rounded-lg ${errors.workTitle ? 'border-red-500' : ''}`} />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Category</label>
                                <select name="workCategory" value={formData.workCategory} onChange={handleInputChange} className="w-full p-2.5 text-sm border rounded-lg">
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Author Name</label>
                                <input name="authorName" value={formData.authorName} onChange={handleInputChange} placeholder="Author Name" className={`w-full p-2.5 text-sm border rounded-lg ${errors.authorName ? 'border-red-500' : ''}`} />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Status</label>
                                <select name="publicationStatus" value={formData.publicationStatus} onChange={handleInputChange} className="w-full p-2.5 text-sm border rounded-lg">
                            ...
                            {formData.publicationStatus === 'PUBLISHED' && (
                                <div className="animate-in fade-in">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">First Publication Date</label>
                                    <input type="date" name="publicationDate" value={formData.publicationDate} onChange={handleInputChange} className={`w-full p-2.5 text-sm border rounded-lg ${errors.publicationDate ? 'border-red-500' : ''}`} />
                                </div>
                            )}
                        </div>
                    </div>
            ...
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Upload size={16} /> UPLOAD WORK</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                { id: 'WORK', label: 'Copy of Work' },
                                { id: 'NOC', label: 'NOC (If required)' },
            ...
            case 3: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm">REVIEW COPYRIGHT</h3>
                        <div className="space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                            <div className="flex justify-between"><span>Work Title</span><span className="font-bold text-navy">{formData.workTitle}</span></div>
                            <div className="flex justify-between"><span>Category</span><span className="font-bold">{formData.workCategory}</span></div>
                            <div className="flex justify-between"><span>Author</span><span className="font-bold">{formData.authorName}</span></div>
            ...
            case 4: return (
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-lg text-center animate-in zoom-in-95">
                    ...
                    <p className="text-xs text-slate-500 mb-6 font-medium">Copyright Registration + Govt Fee</p>
                    ...
                    <button onClick={submitApplication} disabled={!isTermsAccepted || isSubmitting || isPaymentProcessing} className="w-full py-3 bg-[#043E52] text-white font-bold rounded-xl shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
                        {isSubmitting || isPaymentProcessing ? 'Processing Payment...' : 'Pay & Submit Copyright'}
                    </button>
                </div>
            );
            default: return null;
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8">
                        <h1 className="font-bold text-lg flex items-center gap-2 tracking-tight text-white">
                            <BookOpen className="text-[#ED6E3F]" size={20} fill="#ED6E3F" stroke="none" />
                            Copyright
                        </h1>
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Work Details', 'Upload Files', 'Review', 'Payment'].map((step, i) => (
                            <div key={i}
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-slate-800 text-sm truncate">Copyright</span>
                                </div>
                            ...
                            <h2 className="hidden md:block font-bold text-slate-800 text-lg">
                                {['Work Details', 'Upload Documents', 'Review Submission', 'Payment Summary'][currentStep - 1]}
                            </h2>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <p className="text-gray-500 mt-2 font-medium">Copyright registration has been initiated for your work.</p>
                    ...
                    <div className="w-full lg:w-80 shrink-0">
                        <div className="bg-[#043E52] text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
                            ...
                            <h2 className="font-bold text-xl mb-6 relative z-10 flex items-center gap-2 max-w-[80%]">
                                <Copyright size={18} fill="#ED6E3F" stroke="none" />
                                Copyright Registration
                                <div className="mt-1 block text-[10px] text-[#ED6E3F] font-bold uppercase tracking-wider">Protect Your Work</div>
                            </h2>
                            <div className="space-y-3 relative z-10">
                                {['Work Details', 'Upload Files', 'Review', 'Payment'].map((s, i) => (
                    ...
                    <div className="flex-1">
                        {isSuccess ? (
                            <div className="bg-white p-12 rounded-3xl shadow-xl border border-gray-100 text-center">
                                ...
                                <h2 className="text-3xl font-black text-navy mb-4">Work Protected!</h2>
                                <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">Your copyright registration has been successfully submitted to the registry.</p>
