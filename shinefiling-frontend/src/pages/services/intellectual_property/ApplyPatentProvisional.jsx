...
import {
    ...
    ArrowLeft, ArrowRight, IndianRupee, Lightbulb, AlertTriangle, X
} from 'lucide-react';
import { uploadFile, submitPatentFiling } from '../../../api';
import { useRazorpay } from '../../../hooks/useRazorpay';

const ApplyPatentProvisional = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const [planType, setPlanType] = useState(() => {
        ...
        return ['provisional_basic', 'provisional_std', 'provisional_premium', 'standard'].includes(target) ? target : 'standard';
    });

    const planFromUrl = searchParams.get('plan');
    useEffect(() => {
        const targetPlan = (planProp || planFromUrl)?.toLowerCase();
        const validPlan = ['provisional_basic', 'provisional_std', 'provisional_premium', 'standard'].includes(targetPlan) ? targetPlan : 'standard';
    ...
    // Plans
    const plans = {
        provisional_basic: { title: "Filing Only", serviceFee: 4999 },
        provisional_std: { title: "Drafting + Filing", serviceFee: 8999 },
        provisional_premium: { title: "Search + File", serviceFee: 13999 },
        standard: { title: "Drafting + Filing", serviceFee: 8999 },
    };
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${billDetails.planName} - Patent Provisional Filing`,
            prefill: {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Lightbulb size={16} className="text-purple-500" /> INVENTION DETAILS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Title of Invention</label>
                                <input name="inventionTitle" value={formData.inventionTitle} onChange={handleInputChange} placeholder="e.g. Unique Mechanism for Portable Desalination" className={`w-full p-2.5 text-sm border rounded-lg ${errors.inventionTitle ? 'border-red-500' : ''}`} />
                            </div>
                            <div>
                            ...
                            <div className="flex items-center px-4 border rounded-lg bg-purple-50 border-purple-100">
                                <span className="text-xs font-bold text-purple-700">Filing Type: Provisional Patent</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Upload size={16} /> TECHNICAL DISCLOSURE</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
            ...
            case 3: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm">REVIEW APPLICATION</h3>
                        <div className="space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                            ...
                            <div className="flex justify-between"><span>Filing Type</span><span className="font-bold text-purple-600">PROVISIONAL</span></div>
            ...
            case 4: return (
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-lg text-center animate-in zoom-in-95">
                    ...
                    <h3 className="text-xl font-bold text-navy mb-1">Final Payment</h3>
                    <p className="text-xs text-slate-500 mb-6 font-medium">Professional + Govt Filing Fees</p>
                    ...
                    <button onClick={submitApplication} disabled={!isTermsAccepted || isSubmitting || isPaymentProcessing} className="w-full py-3 bg-[#043E52] text-white font-bold rounded-xl shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
                        {isSubmitting || isPaymentProcessing ? 'Processing Payment...' : 'Pay & Submit Application'}
                    </button>
                </div>
            );
            default: return null;
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8">
                        <h1 className="font-bold text-lg flex items-center gap-2 tracking-tight text-white">
                            <Lightbulb className="text-[#ED6E3F]" size={20} fill="#ED6E3F" stroke="none" />
                            Provisional Patent
                        </h1>
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Invention Details', 'Technical Disclosure', 'Review', 'Payment'].map((step, i) => (
                            <div key={i}
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-slate-800 text-sm truncate">Provisional Patent</span>
                                </div>
                            ...
                            <h2 className="hidden md:block font-bold text-slate-800 text-lg">
                                {['Invention Details', 'Technical Disclosure', 'Review', 'Payment Summary'][currentStep - 1]}
                            </h2>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <h2 className="text-2xl font-bold text-navy text-slate-800">Filing Initiated!</h2>
                                <p className="text-gray-500 mt-2 font-medium">Provisional patent filing has been started. Our team will contact you shortly.</p>
                    ...
                    <div className="w-full lg:w-80 shrink-0">
                        <div className="bg-[#043E52] text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
                            ...
                            <h2 className="font-bold text-xl mb-6 relative z-10 flex items-center gap-2 max-w-[80%]">
                                <Lightbulb size={18} fill="#ED6E3F" stroke="none" />
                                Provisional Patent Filing
                                <div className="mt-1 block text-[10px] text-[#ED6E3F] font-bold uppercase tracking-wider">Fast-Track Protection</div>
                            </h2>
                            <div className="space-y-3 relative z-10">
                                {['Invention Details', 'Disclosure', 'Review', 'Payment'].map((s, i) => (
                    ...
                    <div className="flex-1">
                        {isSuccess ? (
                            <div className="bg-white p-12 rounded-3xl shadow-xl border border-gray-100 text-center">
                                ...
                                <h2 className="text-3xl font-black text-navy mb-4">Application filed!</h2>
                                <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">Your provisional patent application has been initiated successfully.</p>
