...
import {
    ...
    ArrowLeft, ArrowRight, IndianRupee, FileWarning, Clock, AlertTriangle, X, Shield
} from 'lucide-react';
...
const ApplyTrademarkObjection = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const [planType, setPlanType] = useState(() => {
        ...
        return ['consultation', 'standard', 'complex'].includes(target) ? target : 'standard';
    });

    const planFromUrl = searchParams.get('plan');
    useEffect(() => {
        const targetPlan = (planProp || planFromUrl)?.toLowerCase();
        const validPlan = ['consultation', 'standard', 'complex'].includes(targetPlan) ? targetPlan : 'standard';
        if (validPlan !== planType) {
            setPlanType(validPlan);
        }
    }, [planProp, planFromUrl]); // Removed planType from dependencies to prevent redundant checks

    // Plans
    const plans = {
        consultation: { title: "Consultation", serviceFee: 999 },
        standard: { title: "Standard Reply", serviceFee: 2999 },
        complex: { title: "Complex Case", serviceFee: 4999 },
    };
    ...
    const billDetails = useMemo(() => {
        ...
        return {
            ...
            tax,
            gst,
            taxesTotal: platformFee + tax + gst, // 18% total professional taxes
    ...
    const validateStep = (step) => {
        ...
        if (step === 1) {
            ...
            if (!formData.examinationReportDate) { newErrors.examinationReportDate = "Required"; isValid = false; }
            if (!formData.applicantResponse) { newErrors.applicantResponse = "Required"; isValid = false; }
        }
        setErrors(newErrors);
        return isValid;
    };
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${billDetails.planName} - Trademark Objection Reply`,
            prefill: {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><FileWarning size={16} className="text-red-500" /> OBJECTION DETAILS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Objection Type</label>
                                <select name="objectionType" value={formData.objectionType} onChange={handleInputChange} className="w-full p-2.5 text-sm border rounded-lg">
                                    <option value="SECTION_9">Section 9 (Absolute Grounds)</option>
                                    <option value="SECTION_11">Section 11 (Relative Grounds)</option>
                                    <option value="FORMALITIES">Formalities / Missing Docs</option>
                                    <option value="OTHERS">Others</option>
                                </select>
                            </div>
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Examination Report Date</label>
                                <input type="date" name="examinationReportDate" value={formData.examinationReportDate} onChange={handleInputChange} className={`w-full p-2.5 text-sm border rounded-lg ${errors.examinationReportDate ? 'border-red-500' : ''}`} />
                                <p className="text-[10px] text-red-500 mt-1 font-bold italic">* Reply deadline: 30 days from this date.</p>
                            </div>
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Initial Comments</label>
                                <textarea name="applicantResponse" value={formData.applicantResponse} onChange={handleInputChange} rows="3" placeholder="Explain your side briefly..." className={`w-full p-2.5 text-sm border rounded-lg ${errors.applicantResponse ? 'border-red-500' : ''}`} />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Upload size={16} /> EVIDENCE & DOCUMENTS</h3>
                        <div className="bg-red-50 p-4 rounded-xl border border-red-100 mb-4">
                            <label className="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" name="requestHearing" checked={formData.requestHearing} onChange={handleInputChange} className="w-4 h-4 text-red-600 rounded" />
                                <div>
                                    <span className="text-sm font-bold text-slate-800 block leading-tight">Request Hearing Directly?</span>
                                    <span className="text-[10px] text-slate-500 block mt-0.5">Check if you wish to skip written response.</span>
                                </div>
                            </label>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                { id: 'REPORT', label: 'Examination Report' },
                                { id: 'EVIDENCE', label: 'Usage Evidence (Bills/Logo)' }
                            ].map((doc) => (
            ...
            case 3: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm">REVIEW SUBMISSION</h3>
                        <div className="space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                            ...
                            <div className="flex justify-between"><span>Type</span><span className="font-bold uppercase text-[10px]">{formData.objectionType}</span></div>
                            <div className="flex justify-between"><span>Report Date</span><span className="font-bold">{formData.examinationReportDate}</span></div>
                            <div className="flex justify-between pt-2 border-t mt-2"><span>Plan chosen</span><span className="font-bold text-orange-600">{billDetails.planName}</span></div>
                        </div>
                    </div>
                </div>
            );
            case 4: return (
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-lg text-center animate-in zoom-in-95">
                    ...
                    <p className="text-xs text-slate-500 mb-6 font-medium">Objection Reply Professional Fee</p>
                    <div className="bg-slate-50 p-4 rounded-xl mb-6 space-y-2 text-xs text-left shadow-inner border border-slate-100">
                        <div className="flex justify-between items-center group">
                            <span className="text-slate-500 group-hover:text-slate-800 transition-colors">Professional Service Fee</span>
                            <span className="font-bold text-navy">₹{billDetails.base.toLocaleString()}</span>
                        </div>
                        <div className="h-px bg-slate-200 my-1"></div>
                        <div className="flex justify-between items-center group">
                            <span className="text-slate-400 group-hover:text-slate-600 transition-colors text-[10px]">Processing & Platform Fee (3%)</span>
                            <span className="font-semibold text-slate-700">₹{billDetails.platformFee.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center group">
                            <span className="text-slate-400 group-hover:text-slate-600 transition-colors text-[10px]">Service Tax (6%)</span>
                            <span className="font-semibold text-slate-700">₹{billDetails.tax.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center group">
                            <span className="text-slate-400 group-hover:text-slate-600 transition-colors text-[10px]">GST (9%)</span>
                            <span className="font-semibold text-slate-700">₹{billDetails.gst.toLocaleString()}</span>
                        </div>
                    ...
                    <button onClick={submitApplication} disabled={!isTermsAccepted || isSubmitting || isPaymentProcessing} className="w-full py-3 bg-[#043E52] text-white font-bold rounded-xl shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
                        {isSubmitting || isPaymentProcessing ? 'Processing Payment...' : 'Pay & Submit Reply'}
                    </button>
                </div>
            );
            default: return null;
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8">
                        ...
                        <div className="mt-6 p-5 bg-[#064e66] rounded-2xl border border-white/10 shadow-xl space-y-4">
                            ...
                            <div className="space-y-3 pt-4 border-t border-white/10 relative z-10">
                                ...
                                <div className="flex justify-between items-center text-xs group">
                                    <span className="text-gray-300 group-hover:text-white transition-colors">Taxes (18%)</span>
                                    <span className="text-white font-medium font-mono">₹{(billDetails.platformFee + billDetails.tax + billDetails.gst).toLocaleString()}</span>
                                </div>
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Objection Details', 'Upload Documents', 'Review Reply', 'Payment'].map((step, i) => (
                            <div key={i}
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-slate-800 text-sm truncate">Trademark Objection</span>
                                </div>
                                <div className="flex items-center gap-3 bg-slate-50 px-2 py-1.5 rounded-lg border border-slate-100 w-fit">
                                    ...
                                    <div className="flex flex-col leading-none">
                                        <span className="text-[8px] text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Taxes</span>
                            ...
                            <h2 className="hidden md:block font-bold text-slate-800 text-lg">
                                {['Objection Details', 'Docs & Evidence', 'Review Application', 'Payment Summary'][currentStep - 1]}
                            </h2>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <h2 className="text-2xl font-bold text-navy text-slate-800">Reply Filed!</h2>
                                <p className="text-gray-500 mt-2 font-medium">Our legal team will start drafting your reply.</p>
                    ...
                    <div className="w-full lg:w-80 shrink-0">
                        <div className="bg-[#043E52] text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
                            ...
                            <h2 className="font-bold text-xl mb-6 relative z-10 flex items-center gap-2 max-w-[80%]">
                                ...
                                Trademark Objection
                                <div className="mt-1 block text-[10px] text-[#ED6E3F] font-bold uppercase tracking-wider">Expert Reply Drafting</div>
                            </h2>
                            <div className="space-y-3 relative z-10">
                                {['Objection Details', 'Evidence', 'Review', 'Payment'].map((s, i) => (
                    ...
                    <div className="flex-1">
                        {isSuccess ? (
                            <div className="bg-white p-12 rounded-3xl shadow-xl border border-gray-100 text-center">
                                ...
                                <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">Your request for trademark objection reply has been successfully placed.</p>
