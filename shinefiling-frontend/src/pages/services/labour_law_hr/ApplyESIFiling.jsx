...
import {
    ...
    ArrowLeft, ArrowRight, IndianRupee, AlertTriangle, Activity, X
} from 'lucide-react';
import { uploadFile, submitESIFiling } from '../../../api';
import { useRazorpay } from '../../../hooks/useRazorpay';

const ApplyESIFiling = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const plans = {
        nil: { title: "NIL Return Filing", serviceFee: 299 },
        standard: { title: "Standard Filing", serviceFee: 499 },
        combo: { title: "PF + ESI Combo", serviceFee: 899 },
    };
    ...
    const validateStep = (step) => {
        ...
        let isValid = true;
        if (step === 1) {
            if (!formData.esicCode) { newErrors.esicCode = "Required"; isValid = false; }
            if (!formData.filingMonth) { newErrors.filingMonth = "Required"; isValid = false; }
        }
        setErrors(newErrors);
        return isValid;
    };
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${billDetails.planName} - ESI Filing`,
            prefill: {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Activity size={18} className="text-pink-600" /> FILING DETAILS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">ESIC Establishment Code</label>
                                <input name="esicCode" value={formData.esicCode} onChange={handleInputChange} placeholder="e.g. 52000213..." className="w-full p-2.5 text-sm border rounded-lg" />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Filing Month</label>
                                <input type="month" name="filingMonth" value={formData.filingMonth} onChange={handleInputChange} className="w-full p-2.5 text-sm border rounded-lg" />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Total Wages Paid</label>
                                <input name="totalWages" value={formData.totalWages} onChange={handleInputChange} placeholder="e.g. 250000" className="w-full p-2.5 text-sm border rounded-lg" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Upload size={18} className="text-pink-600" /> REQUIRED DATA</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                { id: 'wageSheet', label: 'Wage Sheet (Excel/CSV)' },
                                { id: 'challan', label: 'Previous Challan (Optional)' }
                            ].map((doc) => (
                                <div key={doc.id} className="border-2 border-dashed p-4 rounded-xl text-center hover:border-pink-200 transition-colors">
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                            <div className="flex justify-between"><span>ESIC Code</span><span className="font-bold text-navy">{formData.esicCode}</span></div>
                            <div className="flex justify-between"><span>Month</span><span className="font-bold">{formData.filingMonth}</span></div>
                            <div className="flex justify-between pt-2 border-t mt-2"><span>Service Plan</span><span className="font-bold text-pink-600">{billDetails.planName}</span></div>
                        </div>
                    </div>
                </div>
            );
            case 4: return (
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-lg text-center animate-in zoom-in-95">
                    <div className="w-16 h-16 bg-pink-50 rounded-full flex items-center justify-center mx-auto mb-4">
                        <IndianRupee size={24} className="text-pink-600" />
                    </div>
                    <h3 className="text-xl font-bold text-navy mb-1">Make Payment</h3>
                    <p className="text-xs text-slate-500 mb-6 font-medium">ESI Filing Professional Fee</p>
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8">
                        <h1 className="font-bold text-lg flex items-center gap-2 tracking-tight text-white">
                            <Activity className="text-[#ED6E3F]" size={20} fill="#ED6E3F" stroke="none" />
                            ESI Filing
                        </h1>
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Filing Details', 'Upload Data', 'Review', 'Payment'].map((step, i) => (
                    ...
                    <div className="min-h-[64px] bg-white border-b flex items-center justify-between px-4 md:px-6 py-2 shrink-0 z-20">
                        <div className="flex flex-col">
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <span className="font-bold text-slate-800 text-sm truncate">ESI Filing</span>
                            ...
                            <h2 className="hidden md:block font-bold text-slate-800 text-lg">{['Filing Details', 'Upload Data', 'Review', 'Payment Summary'][currentStep - 1]}</h2>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <p className="text-gray-500 mt-2 font-medium">We have received your filing details for {formData.filingMonth}.</p>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="w-full lg:w-80 shrink-0">
                        <div className="bg-[#043E52] text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-3xl -mr-10 -mt-10"></div>
                            <h2 className="font-bold text-xl mb-6 relative z-10 flex items-center gap-2 max-w-[80%]"><Activity size={18} fill="#ED6E3F" stroke="none" /> ESI Filing</h2>
                            <div className="space-y-3 relative z-10">
                                {['Filing Details', 'Upload Data', 'Review', 'Payment'].map((s, i) => (
                    ...
                    <div className="flex-1">
                        {isSuccess ? (
                            <div className="bg-white p-12 rounded-3xl shadow-xl border border-gray-100 text-center">
                                ...
                                <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">Your ESI filing request has been submitted.</p>
