...
const ApplyESIRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const plans = {
        combo_reg: { title: "PF + ESI Registration", serviceFee: 3499 },
        standard: { title: "Standard Registration", serviceFee: 1999 },
        combo_filing: { title: "Registration + Return", serviceFee: 3999 },
    };
    ...
    const validateStep = (step) => {
        ...
        let isValid = true;
        if (step === 1) {
            if (!formData.establishmentName) { newErrors.establishmentName = "Required"; isValid = false; }
            if (!formData.employerName) { newErrors.employerName = "Required"; isValid = false; }
        }
        setErrors(newErrors);
        return isValid;
    };
    ...
    const submitApplication = async () => {
        ...
        const phone = userObj?.phone || formData.contactNumber || '';

        processPayment({
            amount: billDetails.total,
            description: `Payment for ${billDetails.planName} - ESI Registration`,
            prefill: {
                name: userObj?.name || formData.employerName || "Customer",
                email: email,
                contact: phone
            },
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Users size={18} className="text-blue-600" /> BUSINESS DETAILS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Establishment Name</label>
                                <input name="establishmentName" value={formData.establishmentName} onChange={handleInputChange} placeholder="e.g. ABC Technologies Pvt Ltd" className="w-full p-2.5 text-sm border rounded-lg" />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Entity Type</label>
                                <select name="establishmentType" value={formData.establishmentType} onChange={handleInputChange} className="w-full p-2.5 text-sm border rounded-lg">
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Employer Name</label>
                                <input name="employerName" value={formData.employerName} onChange={handleInputChange} className="w-full p-2.5 text-sm border rounded-lg" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Upload size={18} className="text-blue-600" /> DOCUMENTS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                { id: 'proof', label: 'Business Proof (COI/Deed)' },
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                            <div className="flex justify-between"><span>Name</span><span className="font-bold text-navy">{formData.establishmentName}</span></div>
                            <div className="flex justify-between"><span>Type</span><span className="font-bold">{formData.establishmentType}</span></div>
            ...
            case 4: return (
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-lg text-center animate-in zoom-in-95">
                    ...
                    <p className="text-xs text-slate-500 mb-6 font-medium">ESI Registration Professional Fee</p>
                    ...
                    <div className="min-h-[64px] bg-white border-b flex items-center justify-between px-4 md:px-6 py-2 shrink-0 z-20">
                        <div className="flex flex-col">
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <span className="font-bold text-slate-800 text-sm truncate">ESI Registration</span>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <p className="text-gray-500 mt-2 font-medium">We have received your registration details for {formData.establishmentName}.</p>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="w-full lg:w-80 shrink-0">
                        <div className="bg-[#043E52] text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-3xl -mr-10 -mt-10"></div>
                            <h2 className="font-bold text-xl mb-6 relative z-10 flex items-center gap-2 max-w-[80%]"><Users size={18} fill="#ED6E3F" stroke="none" /> ESI Registration</h2>
                    ...
                    <div className="flex-1">
                        {isSuccess ? (
                            <div className="bg-white p-12 rounded-3xl shadow-xl border border-gray-100 text-center">
                                ...
                                <h2 className="text-3xl font-black text-navy mb-4">Registration Received!</h2>
                                <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">Your ESI registration request has been submitted.</p>
