...
const ApplyPFFiling = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const validateStep = (step) => {
        ...
        let isValid = true;
        if (step === 1) {
            if (!formData.pfEstablishmentCode) { newErrors.pfEstablishmentCode = "Required"; isValid = false; }
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${billDetails.planName} - PF Return Filing`,
            prefill: {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Calendar size={18} className="text-blue-600" /> FILING DETAILS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Establishment Code</label>
                                <input name="pfEstablishmentCode" value={formData.pfEstablishmentCode} onChange={handleInputChange} placeholder="e.g. MH/BAN/00000..." className="w-full p-2.5 text-sm border rounded-lg" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="number" name="employeeCount" value={formData.employeeCount} onChange={handleInputChange} placeholder="e.g. 25" className="w-full p-2.5 text-sm border rounded-lg" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Upload size={18} className="text-blue-600" /> REQUIRED DATA</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                { id: 'SALARY_SHEET', label: 'Salary Sheet (Excel/CSV)' },
                                { id: 'PREV_ECR', label: 'Previous ECR (Optional)' }
                            ].map((doc) => (
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                            <div className="flex justify-between"><span>Code</span><span className="font-bold text-navy">{formData.pfEstablishmentCode}</span></div>
            ...
            case 4: return (
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-lg text-center animate-in zoom-in-95">
                    ...
                    <p className="text-xs text-slate-500 mb-6 font-medium">PF Return Filing Fee</p>
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8">
                        <h1 className="font-bold text-lg flex items-center gap-2 tracking-tight text-white">
                            <Calendar className="text-[#ED6E3F]" size={20} fill="#ED6E3F" stroke="none" />
                            PF Filing
                        </h1>
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Filing Details', 'Required Data', 'Review', 'Payment'].map((step, i) => (
                    ...
                    <div className="min-h-[64px] bg-white border-b flex items-center justify-between px-4 md:px-6 py-2 shrink-0 z-20">
                        <div className="flex flex-col">
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <span className="font-bold text-slate-800 text-sm truncate">PF Filing</span>
                            ...
                            <h2 className="hidden md:block font-bold text-slate-800 text-lg">{['Filing Details', 'Required Data', 'Review', 'Payment Summary'][currentStep - 1]}</h2>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <p className="text-gray-500 mt-2 font-medium">We have received your PF Filing details for {formData.filingMonth}.</p>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="w-full lg:w-80 shrink-0">
                        <div className="bg-[#043E52] text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-3xl -mr-10 -mt-10"></div>
                            <h2 className="font-bold text-xl mb-6 relative z-10 flex items-center gap-2 max-w-[80%]"><Calendar size={18} fill="#ED6E3F" stroke="none" /> PF Return Filing</h2>
                            <div className="space-y-3 relative z-10">
                                {['Filing Details', 'Required Data', 'Review', 'Payment'].map((s, i) => (
                    ...
                    <div className="flex-1">
                        {isSuccess ? (
                            <div className="bg-white p-12 rounded-3xl shadow-xl border border-gray-100 text-center">
                                ...
                                <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">Your PF filing request has been submitted.</p>
