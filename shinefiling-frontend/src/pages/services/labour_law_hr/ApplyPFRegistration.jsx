...
import {
    ...
    ArrowLeft, ArrowRight, IndianRupee, Lightbulb, AlertTriangle, Users, X
} from 'lucide-react';
import { uploadFile, submitPFRegistration } from '../../../api';
import { useRazorpay } from '../../../hooks/useRazorpay';

const ApplyPFRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    // Plans
    const plans = {
        voluntary: { title: "Voluntary Registration", serviceFee: 1499 },
        standard: { title: "Standard Registration", serviceFee: 1999 },
        combo: { title: "Registration + Return", serviceFee: 3999 }
    };
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${billDetails.planName} - PF Registration`,
            prefill: {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Users size={18} className="text-blue-600" /> ESTABLISHMENT DETAILS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Business Name</label>
                                <input name="businessName" value={formData.businessName} onChange={handleInputChange} placeholder="e.g. ABC Technologies Pvt Ltd" className={`w-full p-2.5 text-sm border rounded-lg ${errors.businessName ? 'border-red-500' : ''}`} />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Entity Type</label>
                                <select name="businessType" value={formData.businessType} onChange={handleInputChange} className="w-full p-2.5 text-sm border rounded-lg">
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Total Employees</label>
                                <input type="number" name="employeeCount" value={formData.employeeCount} onChange={handleInputChange} placeholder="e.g. 20" className={`w-full p-2.5 text-sm border rounded-lg ${errors.employeeCount ? 'border-red-500' : ''}`} />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Upload size={18} className="text-blue-600" /> REQUIRED DOCUMENTS</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                { id: 'PAN', label: 'Establishment PAN' },
                                { id: 'ADDRESS_PROOF', label: 'Address Proof' },
                                { id: 'INC_CERT', label: 'Incorporation Certificate' },
                                { id: 'CANCELLED_CHEQUE', label: 'Cancelled Cheque' }
                            ].map((doc) => (
                                <div key={doc.id} className="border-2 border-dashed p-4 rounded-xl text-center hover:border-blue-200 transition-colors">
            ...
            case 3: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm">REVIEW DETAILS</h3>
                        <div className="space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                            <div className="flex justify-between"><span>Business Name</span><span className="font-bold text-navy">{formData.businessName}</span></div>
                            <div className="flex justify-between"><span>Type</span><span className="font-bold">{formData.businessType}</span></div>
                            <div className="flex justify-between"><span>Employees</span><span className="font-bold">{formData.employeeCount}</span></div>
                            <div className="flex justify-between pt-2 border-t mt-2"><span>Service Plan</span><span className="font-bold text-blue-600">{billDetails.planName}</span></div>
                        </div>
                    </div>
                </div>
            );
            case 4: return (
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-lg text-center animate-in zoom-in-95">
                    ...
                    <p className="text-xs text-slate-500 mb-6 font-medium">PF Registration Fee</p>
                    ...
                    <button onClick={submitApplication} disabled={!isTermsAccepted || isSubmitting || isPaymentProcessing} className="w-full py-3 bg-[#043E52] text-white font-bold rounded-xl shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
                        {isSubmitting || isPaymentProcessing ? 'Processing Payment...' : 'Pay & Submit Registration'}
                    </button>
                </div>
            );
            default: return null;
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Business Details', 'Required Docs', 'Review', 'Payment'].map((step, i) => (
                    ...
                    <div className="min-h-[64px] bg-white border-b flex items-center justify-between px-4 md:px-6 py-2 shrink-0 z-20">
                        <div className="flex flex-col">
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <span className="font-bold text-slate-800 text-sm truncate">PF Registration</span>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <h2 className="text-2xl font-bold text-slate-800">Registration Filed!</h2>
                                <p className="text-gray-500 mt-2 font-medium">We have received your details for PF Registration.</p>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="w-full lg:w-80 shrink-0">
                        <div className="bg-[#043E52] text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-3xl -mr-10 -mt-10"></div>
                            <h2 className="font-bold text-xl mb-6 relative z-10 flex items-center gap-2 max-w-[80%]"><Users size={18} fill="#ED6E3F" stroke="none" /> PF Registration</h2>
                    ...
                    <div className="flex-1">
                        {isSuccess ? (
                            <div className="bg-white p-12 rounded-3xl shadow-xl border border-gray-100 text-center">
                                ...
                                <h2 className="text-3xl font-black text-navy mb-4">Registration Initiated!</h2>
                                <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">Your PF registration application has been submitted.</p>
