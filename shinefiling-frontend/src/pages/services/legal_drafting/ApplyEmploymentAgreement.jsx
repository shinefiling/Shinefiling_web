...
import {
    ...
    Save, Shield, CreditCard, Lock, Info, Award, User, X, Briefcase, Calendar, Scale
} from 'lucide-react';
...
const pricing = {
    basic: { serviceFee: 999, title: "Basic", subtitle: "Interns & Jr. Staff" },
    standard: { serviceFee: 1499, title: "Standard", subtitle: "Full-Time Employees" },
    executive: { serviceFee: 2999, title: "Executive", subtitle: "Directors / C-Suite" }
};

const ApplyEmploymentAgreement = ({ isLoggedIn, isModal = false, onClose, planProp }) => {
    ...
    const validateStep = (currentStep) => {
        ...
        switch (currentStep) {
            ...
            case 2: // Details
                ...
                if (!formData.employeeName) return "Employee Name is required.";
                if (!formData.designation) return "Designation is required.";
                if (!formData.salary) return "Salary Details are required.";
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                                <Scale size={20} className="text-cyan-600" /> SELECT PLAN
                            </h3>
                            <div className="bg-cyan-50 p-4 rounded-xl border border-cyan-100 flex items-start gap-3 mb-6">
                                <Info className="text-cyan-600 mt-0.5 shrink-0" size={18} />
                                <p className="text-sm text-cyan-800">
                                    <strong>Basic</strong> for simple contracts. <strong>Standard</strong> includes detailed IP & Non-Compete. <strong>Executive</strong> for complex compensation packages.
                                </p>
                            </div>
                                ...
                                {Object.entries(pricing).map(([key, val]) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-right">
                                            <div className="text-cyan-600 font-bold">₹{val.serviceFee}</div>
                                            {plan === key && <CheckCircle size={16} className="text-cyan-600 ml-auto mt-1" />}
                                        </div>
                                    </button>
                                ))}
                            </div>
            ...
            case 2: // Details
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                                <Briefcase size={20} className="text-bronze" /> EMPLOYMENT DETAILS
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Employer Name</label>
                                    <input type="text" name="employerName" value={formData.employerName} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" placeholder="Company Name" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Employee Name</label>
                                    <input type="text" name="employeeName" value={formData.employeeName} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Designation</label>
                                    <input type="text" name="designation" value={formData.designation} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" placeholder="e.g. Software Engineer" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Salary (Annual/Monthly)</label>
                                    <input type="text" name="salary" value={formData.salary} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" placeholder="e.g. 5 LPA" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Joining Date</label>
                                    <input type="date" name="joiningDate" value={formData.joiningDate} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Probation Period</label>
                                    <input type="text" name="probationPeriod" value={formData.probationPeriod} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" placeholder="e.g. 3 Months" />
                                </div>
                ...
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            ...
                            <p className="text-sm text-gray-500 mb-4">Optional: Upload Offer Letter draft or Salary Structure.</p>
                            <div className="grid grid-cols-1 gap-4">
                                {[
                                    { id: 'offerLetter', label: 'Offer Letter Draft', icon: FileText },
                                    { id: 'other', label: 'Other Requirements', icon: FileText }
                                ].map((doc) => (
                                    <div key={doc.id} className="border border-dashed p-4 rounded-lg flex justify-between items-center group hover:border-cyan-300 transition-colors bg-gray-50">
                                        <div className="flex items-center gap-4">
                                            <doc.icon size={20} className="text-gray-400 group-hover:text-cyan-500" />
                                            <div>
            ...
            case 4: // Review
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        <div className="w-20 h-20 bg-cyan-50 rounded-full flex items-center justify-center mx-auto mb-6 text-navy">
                            <Shield size={32} />
                        </div>
                        ...
                        <div className="max-w-xs mx-auto bg-gray-50 p-6 rounded-2xl mb-8 border border-gray-200 text-left">
                            <div className="space-y-3 text-sm">
                                ...
                                <div className="flex justify-between"><span>Employee</span><span className="font-bold">{formData.employeeName}</span></div>
                                <div className="flex justify-between"><span>Designation</span><span className="font-bold">{formData.designation}</span></div>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col justify-center">
                        <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                            <div className="flex items-center gap-2 truncate">
                                <span className="font-bold text-slate-800 text-sm truncate">Employment Agreement</span>
                            </div>
