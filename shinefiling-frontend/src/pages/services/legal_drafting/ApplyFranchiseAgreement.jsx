...
import {
    ...
    Save, Shield, CreditCard, Lock, Info, Award, User, X, ShoppingBag, MapPin, Globe
} from 'lucide-react';
...
const pricing = {
    review: { serviceFee: 1999, title: "Review", subtitle: "Review Existing Draft" },
    standard: { serviceFee: 3999, title: "Standard", subtitle: "Unit Franchise Draft" },
    master: { serviceFee: 9999, title: "Master Draft", subtitle: "For Master Franchisees" }
};

const ApplyFranchiseAgreement = ({ isLoggedIn, isModal = false, onClose, planProp }) => {
    ...
    const validateStep = (currentStep) => {
        ...
        switch (currentStep) {
            case 1: // Plan Selection
                return null;
            case 2: // Details
                if (!formData.franchisorName) return "Franchisor Name is required.";
                if (!formData.franchiseeName) return "Franchisee Name is required.";
                if (!formData.franchiseFee) return "Franchise Fee is required.";
    ...
    const handleSubmit = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${billDetails.planName} - Franchise Agreement Drafting`,
            prefill: {
            ...
            onSuccess: async (response) => {
                ...
                try {
                    ...
                    const uploadPromises = Object.entries(documents).map(async ([key, file]) => {
                        if (file) {
                            try {
                                const uploadRes = await uploadFile(file, 'franchise-agreement');
                                return {
                                    id: key,
    ...
    const steps = ['Select Plan', 'Franchise Details', 'Documents', 'Review', 'Payment'];
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                                <Globe size={20} className="text-rose-600" /> SELECT PLAN
                            </h3>
                            <div className="bg-rose-50 p-4 rounded-xl border border-rose-100 flex items-start gap-3 mb-6">
                                ...
                                <p className="text-sm text-rose-800">
                                    <strong>Review</strong> for vetting third-party contracts. <strong>Standard</strong> for drafting your brand's FOFO agreement. <strong>Master</strong> for complex master franchise rights.
                                </p>
                            </div>
                                ...
                                {Object.entries(pricing).map(([key, val]) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-right">
                                            <div className="text-rose-600 font-bold">₹{val.serviceFee}</div>
                                            {plan === key && <CheckCircle size={16} className="text-rose-600 ml-auto mt-1" />}
                                        </div>
                                    </button>
                                ))}
                            </div>
            ...
            case 2: // Details
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                                <ShoppingBag size={20} className="text-bronze" /> FRANCHISE DETAILS
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Franchisor Name (Brand)</label>
                                    <input type="text" name="franchisorName" value={formData.franchisorName} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" placeholder="Brand / Company Name" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Franchisee Name (Applicant)</label>
                                    <input type="text" name="franchiseeName" value={formData.franchiseeName} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" placeholder="Individual / Entity Name" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Franchise Fee (₹)</label>
                                    <input type="number" name="franchiseFee" value={formData.franchiseFee} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Royalty (%)</label>
                                    <input type="text" name="royalty" value={formData.royalty} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white" placeholder="e.g. 5% on Gross Sales" />
                                </div>
                                <div className="space-y-1 md:col-span-2">
                                    <label className="text-xs font-bold text-gray-500">Territory / Location</label>
                                    <textarea name="territory" value={formData.territory} onChange={handleChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white resize-none" rows="2" placeholder="e.g. Exclusive Rights for South Delhi" />
                                </div>
                                <div className="space-y-1">
            ...
            case 3: // Documents
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                                <Upload size={20} className="text-rose-500" /> UPLOAD DOCUMENTS
                            </h3>
                            <p className="text-sm text-gray-500 mb-4">Optional: Upload existing draft or brand guidelines.</p>
                            <div className="grid grid-cols-1 gap-4">
                                {[
                                    { id: 'existingDraft', label: 'Existing Draft (If Any)', icon: FileText },
                                    { id: 'brandGuidelines', label: 'Brand Guidelines / Term Sheet', icon: FileText }
                                ].map((doc) => (
                                    <div key={doc.id} className="border border-dashed p-4 rounded-lg flex justify-between items-center group hover:border-rose-300 transition-colors bg-gray-50">
                                        <div className="flex items-center gap-4">
                                            <doc.icon size={20} className="text-gray-400 group-hover:text-rose-500" />
                                            <div>
            ...
            case 4: // Review
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        <div className="w-20 h-20 bg-rose-50 rounded-full flex items-center justify-center mx-auto mb-6 text-navy">
                            <Shield size={32} />
                        </div>
                        ...
                        <div className="max-w-xs mx-auto bg-gray-50 p-6 rounded-2xl mb-8 border border-gray-200 text-left">
                            <div className="space-y-3 text-sm">
                                ...
                                <div className="flex justify-between"><span>Franchisor</span><span className="font-bold">{formData.franchisorName}</span></div>
                                <div className="flex justify-between"><span>Franchisee</span><span className="font-bold">{formData.franchiseeName}</span></div>
                                <div className="flex justify-between"><span>Fee</span><span className="font-bold">₹{formData.franchiseFee}</span></div>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col justify-center">
                        <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                            <div className="flex items-center gap-2 truncate">
                                <span className="font-bold text-slate-800 text-sm truncate">Franchise Agreement</span>
                            </div>
