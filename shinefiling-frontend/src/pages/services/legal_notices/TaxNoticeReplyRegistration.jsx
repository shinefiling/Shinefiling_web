...
const TaxNoticeReplyRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const validatePlan = (plan) => ['basic', 'standard', 'appeal'].includes(plan?.toLowerCase()) ? plan.toLowerCase() : 'standard';
    ...
    const plans = {
        basic: { price: 999, title: 'Basic IT Notice', features: ["Defective Return", "Demand Confirmation", "Simple Rectification"], color: 'bg-white border-slate-200' },
        standard: { price: 1999, title: 'GST / IT Regular', features: ["GST Discrepancy Reply", "Limited Scrutiny", "ITC Calculation"], recommended: true, color: 'bg-[#043E52] text-white border-gray-700' },
        appeal: { price: 4999, title: 'Appeal / Complex', features: ["Appeal Filing", "Personal Hearing", "Senior CA Consultation"], color: 'bg-purple-50 border-purple-200' }
    };
    ...
    const validateStep = (step) => {
        ...
        if (step === 1) {
            ...
            if (!formData.noticeDate) { newErrors.noticeDate = "Required"; isValid = false; }
            if (!formData.referenceNumber) { newErrors.referenceNumber = "Required"; isValid = false; }
        }
        if (step === 2) {
    ...
    const submitApplication = async () => {
        ...
        const phone = userObj?.mobile || '';

        processPayment({
            amount: billDetails.total,
            description: `Payment for Tax Notice Reply - ${plans[selectedPlan].title}`,
            prefill: {
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Notice Info', 'Entity Details', 'Upload Notice', 'Review', 'Payment'].map((step, i) => (
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-slate-800 text-sm truncate">Tax Notice Reply</span>
                                </div>
        ...
        switch (currentStep) {
                ...
                return (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Landmark size={16} className="text-[#015A62]" /> NOTICE INFORMATION</h3>
                            <div className="grid md:grid-cols-2 gap-3">
                                <div><label className="text-xs font-bold text-gray-500 mb-1 block">Notice Type</label>
                                    <select name="noticeType" value={formData.noticeType} onChange={handleInputChange} className="w-full p-2 text-sm border border-gray-200 rounded-lg bg-white">
                                ...
                                <div><label className="text-xs font-bold text-gray-500 mb-1 block">Notice Section (e.g. 143(1), ASMT-10)</label>
                                    <input name="noticeSection" value={formData.noticeSection} onChange={handleInputChange} className="w-full p-2 text-sm border border-gray-200 rounded-lg" /></div>
                                <div><label className="text-xs font-bold text-gray-500 mb-1 block">Notice Date</label>
                                    <input type="date" name="noticeDate" value={formData.noticeDate} onChange={handleInputChange} className={`w-full p-2 text-sm border rounded-lg ${errors.noticeDate ? 'border-red-500' : 'border-gray-200'}`} /></div>
                                <div><label className="text-xs font-bold text-gray-500 mb-1 block">DIN / Reference Number</label>
                                    <input name="referenceNumber" value={formData.referenceNumber} onChange={handleInputChange} className={`w-full p-2 text-sm border rounded-lg ${errors.referenceNumber ? 'border-red-500' : 'border-gray-200'}`} /></div>
                            </div>
                        </div>
                    </div>
                );
            case 2:
                return (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2"><Building size={16} className="text-[#ED6E3F]" /> ENTITY DETAILS</h3>
                            <div className="grid gap-3">
                                <div><label className="text-xs font-bold text-gray-500 mb-1 block">Business Name / Taxpayer Name</label>
                                    <input name="businessName" value={formData.businessName} onChange={handleInputChange} className={`w-full p-2 text-sm border rounded-lg ${errors.businessName ? 'border-red-500' : 'border-gray-200'}`} /></div>
                                <div><label className="text-xs font-bold text-gray-500 mb-1 block">Assessment Year (AY)</label>
                                    <input name="assessmentYear" value={formData.assessmentYear} onChange={handleInputChange} placeholder="e.g. 2024-25" className="w-full p-2 text-sm border border-gray-200 rounded-lg" /></div>
                            </div>
                        </div>
                    </div>
                );
            case 3:
                return (
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm text-center animate-in fade-in slide-in-from-right-4">
                        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2 justify-center"><Upload size={16} className="text-[#015A62]" /> UPLOAD NOTICE COPY</h3>
            ...
            case 4:
                return (
                    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 text-sm animate-in zoom-in-95">
                        ...
                        <div className="p-4 bg-gray-50 rounded-lg space-y-2 text-sm uppercase">
                            ...
                            <div className="flex justify-between pt-2 border-t"><span>Total</span><span className="font-bold text-navy">₹{billDetails.total.toLocaleString()}</span></div>
                        </div>
                    </div>
                );
            case 5:
