...
import {
    ...
    ArrowLeft, ArrowRight, IndianRupee, MapPin, Factory, Droplets, AlertTriangle, X, Shield, Check
} from 'lucide-react';
import { uploadFile, submitPollutionControl } from '../../../api';
import { useRazorpay } from '../../../hooks/useRazorpay';
import { useMemo } from 'react';

const PollutionControlRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    // Dynamic Pricing
    const pricing = {
        advisory: { price: 1999, title: "Expert Pollution Advisory" },
        cte: { price: 12499, title: "Consent to Establish" },
        cto: { price: 14999, title: "Consent to Operate" }
    };

    const currentPricing = pricing[planProp] || (formData.businessCategory === 'RED' ? { price: 24999, title: "High Impact Consent" } : pricing.cte);
    ...
    useEffect(() => {
        ...
        if (plan) {
            setPlanType(plan.toUpperCase());
            setFormData(prev => ({ ...prev, certificateType: plan.toUpperCase() }));
        }
    }, [searchParams]);
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${currentPricing.title} - Pollution Control`,
            prefill: {
            ...
            onSuccess: async (response) => {
                ...
                try {
                    ...
                    await submitPollutionControl(finalPayload);
                    setIsSuccess(true);
                } catch (error) {
                    ...
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                        <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
                            <h3 className="text-sm font-bold text-navy uppercase tracking-widest mb-6 flex items-center gap-2">
                                <Factory size={18} className="text-[#ED6E3F]" /> Industry Profile
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2 md:col-span-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Industry Registered Name</label>
                                    <input type="text" name="industryName" value={formData.industryName} onChange={handleInputChange} placeholder="e.g. ACME CHEMICALS" className={`w-full p-4 bg-gray-50 border ${errors.industryName ? 'border-red-400' : 'border-gray-100'} rounded-xl text-sm focus:bg-white focus:border-[#ED6E3F]/30 outline-none transition-all`} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">MPCB / SPCB State</label>
                                    <select name="state" value={formData.state} onChange={handleInputChange} className={`w-full p-4 bg-gray-50 border ${errors.state ? 'border-red-400' : 'border-gray-100'} rounded-xl text-sm outline-none`}>
                                        ...
                                        <option value="Tamil Nadu">Tamil Nadu (TNPCB)</option>
                                        <option value="Gujarat">Gujarat (GPCB)</option>
                                    </select>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Primary Industry Activity</label>
                                    <input type="text" name="industryActivity" value={formData.industryActivity} onChange={handleActivityChange} placeholder="e.g. Manufacturing" className={`w-full p-4 bg-gray-50 border ${errors.industryActivity ? 'border-red-400' : 'border-gray-100'} rounded-xl text-sm focus:bg-white focus:border-[#ED6E3F]/30 outline-none transition-all`} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Consent Type</label>
                                    <select name="certificateType" value={formData.certificateType} onChange={handleInputChange} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl text-sm outline-none">
                                        <option value="CTE">CTE (Establish)</option>
                                        <option value="CTO">CTO (Operate)</option>
                                    </select>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Investment (in Crores)</label>
                                    <input type="number" step="0.01" name="investmentAmount" value={formData.investmentAmount} onChange={handleInputChange} placeholder="e.g. 5.50" className={`w-full p-4 bg-gray-50 border ${errors.investmentAmount ? 'border-red-400' : 'border-gray-100'} rounded-xl text-sm focus:bg-white focus:border-[#ED6E3F]/30 outline-none transition-all`} />
                                </div>
                            </div>
                        </div>
                    </motion.div>
                ...
                return (
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                        <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="text-sm font-bold text-navy uppercase tracking-widest mb-6 flex items-center gap-2">
                                <Droplets size={18} className="text-[#ED6E3F]" /> Environmental Node
                            </h3>
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Site Physical Address</label>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Daily Water Req (KLD)</label>
                                        <input type="number" step="0.1" name="dailyWaterConsumption" value={formData.dailyWaterConsumption} onChange={handleInputChange} placeholder="e.g. 10.5" className={`w-full p-4 bg-gray-50 border ${errors.dailyWaterConsumption ? 'border-red-400' : 'border-gray-100'} rounded-xl text-sm focus:bg-white focus:border-[#ED6E3F]/30 outline-none transition-all`} />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Daily Effluent Discharge (KLD)</label>
                                        <input type="number" step="0.1" name="dailyEffluentDischarge" value={formData.dailyEffluentDischarge} onChange={handleInputChange} placeholder="e.g. 5.0" className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:bg-white focus:border-[#ED6E3F]/30 outline-none transition-all" />
                                    </div>
                                </div>

                                <div className="bg-orange-50/30 p-6 rounded-2xl border border-orange-100/50 space-y-4">
                                    <h4 className="text-[10px] font-bold text-[#ED6E3F] uppercase tracking-widest mb-2">Compliance Checklist</h4>
                                    {[
                                        { id: 'hasEtpStp', label: 'ETP / STP Installed?' },
                                        { id: 'hasAirPollutionControl', label: 'Air Control Systems?' },
                                        { id: 'generatesHazardousWaste', label: 'Hazardous Waste Gen?' }
                                    ].map(item => (
                                        <label key={item.id} className="flex items-center gap-3 cursor-pointer group">
                                            <input type="checkbox" name={item.id} checked={formData[item.id]} onChange={handleInputChange} className="w-4 h-4 rounded border-gray-300 text-[#ED6E3F] focus:ring-[#ED6E3F]" />
                                            <span className="text-xs font-bold text-navy uppercase tracking-widest group-hover:text-[#ED6E3F] transition-colors">{item.label}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
            ...
            case 3:
                return (
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                        <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="text-sm font-bold text-navy uppercase tracking-widest mb-6 flex items-center gap-2">
                                <Upload size={18} className="text-[#ED6E3F]" /> Operational Vault
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {[
                                    { id: 'SITE_PLAN', label: 'Site Plan', icon: MapPin },
                                    { id: 'PROCESS_FLOW', label: 'Process Flow', icon: Factory },
                                    { id: 'CA_CERT', label: 'CA Certificate', icon: FileText }
                                ].map((doc) => (
                ...
                return (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md mx-auto font-poppins text-navy">
                        <div className="bg-white p-10 rounded-3xl shadow-xl border border-gray-100 text-center relative overflow-hidden">
                            ...
                            <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-8">PCB Compliance Protocol</p>
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8">
                        ...
                        <div className="mt-6 p-5 bg-[#064e66] rounded-2xl border border-white/10 shadow-xl space-y-4 relative overflow-hidden">
                            ...
                            <div className="relative z-10">
                                <p className="text-[10px] uppercase text-gray-300 tracking-widest font-bold mb-1.5 opacity-80">Category: {formData.businessCategory || 'PENDING'}</p>
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Industry Profile', 'Environmental', 'Document Vault', 'Review & Pay'].map((step, i) => (
                            <div key={i}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        ...
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <h2 className="text-2xl font-bold text-navy">Filing Recorded!</h2>
                                <p className="text-gray-500 mt-2">Your industry data is now under department assessment.</p>
                ...
                <div className="flex gap-8">
                    <div className="w-72 hidden lg:block space-y-4">
                        <div className="bg-white p-4 rounded-xl shadow-sm border space-y-2">
                            {['Industry Profile', 'Environmental', 'Documents', 'Review'].map((s, i) => (
