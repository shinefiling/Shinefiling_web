...
const MOAAmendmentRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    const plans = {
        ...
        standard: { price: 2999, title: 'Object Shift' },
        ...
        switch (currentStep) {
                ...
                return (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <h3 className="font-bold text-slate-800 mb-3 text-sm flex items-center gap-2"><Building2 size={16} /> ENTITY DETAILS</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <div className="space-y-1 md:col-span-2">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Amendment Focus</p>
                                    <div className="grid grid-cols-3 gap-2">
                                        {['MOA', 'AOA', 'BOTH'].map(t => (
                                            <button key={t} onClick={() => setFormData({ ...formData, amendmentType: t })} className={`py-2 rounded-lg border text-xs font-bold transition-all ${formData.amendmentType === t ? 'bg-[#043E52] text-white border-[#043E52]' : 'bg-white text-slate-500 border-slate-200 hover:border-[#ED6E3F]'}`}>{t}</button>
                                        ))}
                                    </div>
                                </div>
                                <div className="space-y-1 md:col-span-2">
            ...
            case 2:
                return (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-slate-800 mb-3 text-sm flex items-center gap-2"><BookOpen size={16} /> COMPOSITION REVISION</h3>
                            <div className="space-y-4">
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Current Active Clause</p>
                                    <textarea name="existingClause" value={formData.existingClause} onChange={handleInputChange} rows={3} placeholder="Paste current clause text here..." className="w-full p-3 text-sm border rounded-lg bg-gray-50 italic" />
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Proposed Statutory Clause</p>
            ...
            case 3:
                return (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-slate-800 mb-3 text-sm flex items-center gap-2"><ClipboardList size={16} /> EVIDENCE REPOSITORY</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <div className="border border-dashed p-3 rounded-lg flex justify-between items-center bg-gray-50">
                                    <span className="text-xs text-gray-600">Existing Constitution</span>
                                    <input type="file" onChange={(e) => handleFileUpload(e, 'moa')} className="text-[10px] w-24" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Entity Sync', 'Clause Mapping', 'Evidence Board', 'Review', 'Payment'].map((step, i) => (
                            <div key={i}
                ...
                <div className="flex gap-8">
                    <div className="w-72 hidden lg:block space-y-4">
                        <div className="bg-white p-4 rounded-xl shadow-sm border space-y-2">
                            {['Entity', 'Revision', 'Docs', 'Review', 'Payment'].map((s, i) => (
