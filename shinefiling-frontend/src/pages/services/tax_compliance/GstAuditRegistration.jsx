...
import {
    ...
    Building, ArrowLeft, ArrowRight, Shield, AlertCircle, Lock, IndianRupee, Users, Plus, Trash2, X, Briefcase, MapPin, RefreshCw, PieChart, Landmark
} from 'lucide-react';
import { uploadFile, submitGstAudit } from '../../../api';
import { useRazorpay } from '../../../hooks/useRazorpay';

const GstAuditRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (!isReallyLoggedIn) {
            const plan = searchParams.get('plan') || 'audit_report';
            navigate('/login', { state: { from: `/services/gst-audit/register?plan=${plan}` } });
        }
    ...
    const validatePlan = (plan) => {
        return ['reconciliation', 'audit_report', 'departmental'].includes(plan?.toLowerCase()) ? plan.toLowerCase() : 'audit_report';
    };
    ...
    const plans = {
        reconciliation: {
            ...
            features: ['Pre-Audit Simulation', 'Inconsistency Check', 'ITC Reversal Analysis'],
            color: 'bg-white border-slate-200'
        },
        audit_report: {
            price: 5999,
            title: 'GST Audit / 9C',
            features: ['Books vs GST Recon', 'Turnover Reconciliation', 'GSTR-9C Preparation'],
            recommended: true,
        ...
        departmental: {
            price: 9999,
            title: 'Audit Shield',
            features: ['ADT-01 Notice Reply', 'Officer Representation', 'Adjudication Support'],
            color: 'bg-purple-50 border-purple-200'
        }
    };

    const billDetails = useMemo(() => {
        const plan = plans[selectedPlan] || plans.audit_report;
    ...
    const handleFileUpload = async (e, key) => {
        ...
        if (!file) return;
        try {
            const response = await uploadFile(file, 'gst_audit_docs');
    ...
    const submitApplication = async () => {
        const email = JSON.parse(localStorage.getItem('user'))?.email || formData.userEmail;
        const phone = JSON.parse(localStorage.getItem('user'))?.phone || formData.userPhone;

        processPayment({
            amount: billDetails.total,
            description: `Payment for ${plans[selectedPlan]?.title} - GST Audit Registration`,
            prefill: {
                name: formData.tradeName || "Customer",
                email: email,
                contact: phone
            },
            onSuccess: async (response) => {
                ...
                try {
                    ...
                    const apiResponse = await submitGstAudit(finalPayload);
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            case 2: return (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-3 text-sm flex items-center gap-2"><FileText size={16} /> AUDIT SCOPE</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <select name="financialYear" value={formData.financialYear} onChange={handleInputChange} className="p-2 text-sm border rounded-lg w-full">
                                <option>2023-24</option>
                                <option>2022-23</option>
                            </select>
                            <input type="number" name="turnover" value={formData.turnover} onChange={handleInputChange} placeholder="Audited Turnover (₹)" className={`p-2 text-sm border rounded-lg w-full ${errors.turnover ? 'border-red-500' : ''}`} />
                            <select name="auditType" value={formData.auditType} onChange={handleInputChange} className="md:col-span-2 p-2 text-sm border rounded-lg w-full">
                                <option value="GSTR-9C">GSTR-9C Reconciliation</option>
                                <option value="Department Audit Assistance">Department Audit Assistance</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {['Audited Financials (P&L, BS)', 'Draft GSTR-9', 'Department Notices (if any)'].map((doc, i) => (
            ...
            case 4: return (
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg space-y-2 text-sm">
                        ...
                        <div className="flex justify-between"><span>Financial Year</span><span className="font-bold">{formData.financialYear}</span></div>
                        <div className="flex justify-between"><span>Service Type</span><span className="font-bold text-amber-600">{formData.auditType}</span></div>
                    </div>
                </div>
            );
            case 5: return (
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Entity Info', 'Audit Scope', 'Upload Data', 'Review Info', 'Payment'].map((step, i) => (
                            <div key={i}
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-slate-800 text-sm truncate">GST Audit</span>
                                </div>
                            ...
                            <h2 className="hidden md:block font-bold text-slate-800 text-lg">
                                ...
                                {currentStep === 2 && "Audit Scope Details"}
                ...
                <div className="flex gap-8">
                    <div className="w-72 hidden lg:block space-y-4">
                        <div className="bg-white p-4 rounded-xl shadow-sm border space-y-2">
                            {['Entity Info', 'Audit Scope', 'Upload Data', 'Review Info', 'Payment'].map((s, i) => (
...
export default GstAuditRegistration;
