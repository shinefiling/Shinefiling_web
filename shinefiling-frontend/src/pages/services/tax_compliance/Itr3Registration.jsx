...
const Itr3Registration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (!isLoggedIn) {
            ...
            navigate('/login', { state: { from: `/services/income-tax/itr-3/register?plan=${plan}` } });
        }
    }, [isLoggedIn, navigate, searchParams, isModal]);

    const [currentStep, setCurrentStep] = useState(1);
    const [selectedPlan, setSelectedPlan] = useState(planProp || 'business');
    const [formData, setFormData] = useState({ fullName: '', pan: '', dob: '', itrType: 'ITR-3', assessmentYear: '2024-25', businessName: '', natureOfBusiness: '', turnover: '', auditApplicable: 'No' });
    ...
    const plans = {
        business: { price: 2499, title: 'Proprietorship', features: ["P&L, BS Creation"], color: 'bg-white border-slate-200' },
        trader: { price: 3499, title: 'Traders (F&O)', features: ["Broker Analysis"], recommended: true, color: 'bg-indigo-50 border-indigo-200' },
        audit: { price: 5999, title: 'Audit Case', features: ["CA Certified Audit"], color: 'bg-white border-slate-200' }
    };
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${plans[selectedPlan]?.title} - ITR-3 Filing`,
            prefill: {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            case 2: return (
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                        <h3 className="font-bold text-navy mb-4">Business Details</h3>
                        <input type="text" name="businessName" value={formData.businessName} onChange={handleInputChange} placeholder="Proprietorship Name" className="w-full p-3 rounded-lg border border-gray-200" />
                        <input type="text" name="natureOfBusiness" value={formData.natureOfBusiness} onChange={handleInputChange} placeholder="Nature of Business (e.g. Trading)" className="w-full p-3 rounded-lg border border-gray-200" />
                        <div><label className="text-sm font-bold block mb-1">Annual Turnover (Approx)</label><input type="number" name="turnover" value={formData.turnover} onChange={handleInputChange} className="w-full p-3 border rounded-lg" /></div>
                        <div><label className="text-sm font-bold block mb-1">Is Tax Audit Applicable?</label><select name="auditApplicable" value={formData.auditApplicable} onChange={handleInputChange} className="w-full p-3 border rounded-lg"><option value="No">No</option><option value="Yes">Yes</option></select></div>
                    </div>
                </div>
            );
            case 3: return (
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                    <h3 className="font-bold text-navy mb-4">Financial Documents</h3>
                    {['Bank Statements (All A/cs)', 'Sales/Purchase Register', 'Expenses (Excel)', 'Prior Year Return'].map((d, i) => (
                ...
                return (
                    <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                        ...
                        <div className="p-4 bg-gray-50 rounded-lg space-y-2 text-sm">
                            ...
                            <div className="flex justify-between"><span>Form</span><span className="font-bold">ITR-3 Filing</span></div>
                        </div>
                    </div>
                );
            case 5:
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Personal Info', 'Business Details', 'Documents', 'Review', 'Payment'].map((step, i) => (
                            <div key={i}
                        ...
                        <div className="flex flex-col justify-center">
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-slate-800 text-sm truncate">ITR-3 Filing</span>
                                </div>
                ...
                <div className="flex gap-8">
                    <div className="w-72 hidden lg:block space-y-4">
                        <div className="bg-white p-4 rounded-xl shadow-sm border space-y-2">
                            {['Personal Info', 'Business Details', 'Documents', 'Review', 'Payment'].map((s, i) => (
...
export default Itr3Registration;
