...
const Itr4Registration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    useEffect(() => {
        if (isModal) return;
        if (!isLoggedIn) {
            const plan = searchParams.get('plan') || 'presumptive';
            navigate('/login', { state: { from: `/services/income-tax/itr-4/register?plan=${plan}` } });
        }
    }, [isLoggedIn, navigate, searchParams, isModal]);

    const [currentStep, setCurrentStep] = useState(1);
    const [selectedPlan, setSelectedPlan] = useState(planProp || 'presumptive');
    const [formData, setFormData] = useState({ fullName: '', pan: '', dob: '', itrType: 'ITR-4', assessmentYear: '2024-25', mainSource: 'Business (44AD)', grossTurnover: '', netProfit: '' });
    ...
    const plans = {
        presumptive: { price: 1999, title: 'Presumptive (44AD)', features: ["No Books Needed"], color: 'bg-white border-slate-200' },
        professional: { price: 2999, title: 'Professionals (44ADA)', features: ["Freelancer / Doctor"], recommended: true, color: 'bg-indigo-50 border-indigo-200' },
        transport: { price: 2499, title: 'Transporters (44AE)', features: ["Vehicle Based"], color: 'bg-white border-slate-200' }
    };

    const billDetails = useMemo(() => {
        const plan = plans[selectedPlan] || plans.presumptive;
    ...
    const submitApplication = async () => {
        ...
        processPayment({
            ...
            description: `Payment for ${plans[selectedPlan]?.title} - ITR-4 Filing`,
            prefill: {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            case 2: return (
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                        <h3 className="font-bold text-navy mb-4">Presumptive Scheme</h3>
                        <div><label className="text-sm font-bold block mb-1">Applicable Section</label><select name="mainSource" value={formData.mainSource} onChange={handleInputChange} className="w-full p-3 border rounded-lg"><option value="Business (44AD)">Business (44AD) - 8% / 6%</option><option value="Profession (44ADA)">Profession (44ADA) - 50%</option><option value="Trucks (44AE)">Goods Carriage (44AE)</option></select></div>
                        <div><label className="text-sm font-bold block mb-1">Gross Annual Turnover / Receipts</label><input type="number" name="grossTurnover" value={formData.grossTurnover} onChange={handleInputChange} className="w-full p-3 border rounded-lg" /></div>
                        <div><label className="text-sm font-bold block mb-1">Net Income to Declare (Min % as per section)</label><input type="number" name="netProfit" value={formData.netProfit} onChange={handleInputChange} className="w-full p-3 border rounded-lg" /></div>
                    </div>
                </div>
            );
            case 3: return (
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                    ...
                    {['Bank Statement 1', 'Bank Statement 2', 'Sales Summary (Rough)', 'TDS Certificates (26AS)'].map((d, i) => (
                ...
                return (
                    <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                        ...
                        <div className="p-4 bg-gray-50 rounded-lg space-y-2 text-sm">
                            ...
                            <div className="flex justify-between"><span>Form</span><span className="font-bold">ITR-4 (Sugam)</span></div>
                        </div>
                    </div>
                );
            case 5:
            ...
            <div className="flex flex-col md:flex-row h-[85vh] overflow-hidden bg-white">
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Personal Info', 'Presumptive Income', 'Documents', 'Review', 'Payment'].map((step, i) => (
                            <div key={i}
                        ...
                        <div className="flex flex-col justify-center">
                            <div className="md:hidden flex flex-col gap-1 w-full max-w-[calc(100vw-80px)]">
                                <div className="flex items-center gap-2 truncate">
                                    <span className="font-bold text-slate-800 text-sm truncate">ITR-4 (Sugam)</span>
                                </div>
                            ...
                            <h2 className="hidden md:block font-bold text-slate-800 text-lg">
                                ...
                                {currentStep === 2 && "Presumptive Income"}
                ...
                <div className="flex gap-8">
                    <div className="w-72 hidden lg:block space-y-4">
                        <div className="bg-white p-4 rounded-xl shadow-sm border space-y-2">
                            {['Personal Info', 'Presumptive Income', 'Documents', 'Review', 'Payment'].map((s, i) => (
...
export default Itr4Registration;
